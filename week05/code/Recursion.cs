...
public static class Recursion
{
    ...
    if (word.Length == size)
    {
        results.Add(word); // Base case: desired size reached
        return;
    }
    ...
    // Inicializar diccionario si es nulo
    if (remember == null)
        remember = new Dictionary<int, decimal>();

    // Caso base: 0 escalones = 1 forma de no subir nada
    if (s < 0)
        return 0;
    if (s == 0)
        return 1;
    ...
    decimal totalWays = CountWaysToClimb(s - 1, remember) +
    ...
    // Caso base: no hay más '*', es una combinación válida
    if (index == -1)
    {
        results.Add(pattern);
        return;
